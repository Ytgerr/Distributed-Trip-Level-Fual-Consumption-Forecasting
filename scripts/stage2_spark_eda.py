...
# hdfs path with raw parquet data from stage 1; change if ingestion path changes
WAREHOUSE = "/user/team15/project/warehouse"

# hdfs path for analytical parquet outputs; change if another warehouse layer is required
ANALYTICS_HDFS = "/user/team15/project/analytics"

# hdfs path for small csv outputs; change if report/dashboard exports should go elsewhere
ANALYTICS_CSV_HDFS = "/user/team15/project/analytics_csv"

# spark application name; change only for clearer yarn tracking
SPARK_APP_NAME = "stage2-data-science-eda"
SPARK_LOG_LEVEL = "WARN"

GASOLINE_DENSITY_G_PER_L = (
    745.0  # average gasoline density in g/L; tune if fuel type assumptions change
)

# gasoline stoichiometric air-fuel ratio; use 14.7 for standard gasoline approximation
STOICH_AFR = 14.7

# max valid time gap inside one trip in seconds; lower value removes long telemetry breaks
MAX_VALID_DT_S = 3600.0

# speed below this threshold is treated as stop-go traffic
STOP_GO_SPEED_KMH = 5.0

# speed below this threshold is treated as idling
IDLE_SPEED_KMH = 0.1

# lower bound for speed-bin EDA; change if reverse/invalid speeds should be analyzed
MIN_VALID_SPEED_KMH = 0.0
MAX_VALID_SPEED_KMH = (
    160.0  # upper bound for speed-bin EDA; change if high-speed outliers should be kept
)
...
# minimum trip distance kept in trip-level features; increase to remove short noisy trips
MIN_TRIP_DISTANCE_KM = 0.5

TRIP_FEATURE_SAMPLE_SIZE = (
    5000  # number of longest trips exported as a local csv sample
)
...
def clean_nan_values(df):
    ...
    for col_name, col_type in df.dtypes:
        if col_type in ("double", "float"):
            df = df.withColumn(
                col_name,
                F.when(F.isnan(F.col(col_name)), F.lit(None)).otherwise(
                    F.col(col_name)
                ),
            )
...
def write_small_csv(df, name):
    "Wrute DataFrame to csv"
    path = f"{ANALYTICS_CSV_HDFS}/{name}"
...
def write_output(df, name, write_csv=True):
    "Short-cut function for saving data"
...
vehicles = clean_nan_values(vehicles_raw).withColumn(
    "vehid", F.col("vehid").cast("string")
)

trips = (
    clean_nan_values(trips_raw)
...
trips_enriched = (
    ...
    .withColumn(
        "fuel_rate_est_lhr",
        F.when(F.isnan(F.col("fuel_rate_est_lhr")), F.lit(None)).otherwise(
            F.col("fuel_rate_est_lhr")
        ),
    )
    .withColumn(
        "is_stop_go",
...
existing_missing_features = [
    feature for feature in missing_features if feature in trips.columns
]
...
insight_01 = (
    trips_enriched.groupBy(
        F.coalesce(F.col("vehtype"), F.lit(NO_DATA_LABEL)).alias("vehicle_type")
    )
    .agg(
        F.count("*").alias("sensor_rows"),
...
insight_02 = (
    trips_enriched.where(F.col("fuel_rate_est_lhr").isNotNull())
    ...
    .agg(
        ...
        F.expr(f"percentile_approx(fuel_rate_est_lhr, {0.5})").alias(
            "median_fuel_rate_lhr"
        ),
        F.expr(f"percentile_approx(fuel_rate_est_lhr, {0.9})").alias(
            "p90_fuel_rate_lhr"
        ),
    )
...
driving_mode = (
    F.when(F.col("speed") < STOP_GO_SPEED_KMH, F.lit("stop_go_under_5_kmh"))
...
insight_04 = (
    trips_enriched.where(
        (F.col("speed").isNotNull()) & (F.col("fuel_rate_est_lhr").isNotNull())
    )
...
insight_05 = (
    trips_enriched.where(F.col("oat").isNotNull())
    .withColumn(
        "oat_bin_c",
    ...
    .agg(
        ...
        F.avg("fuel_rate_est_lhr").alias("avg_fuel_rate_lhr"),
        F.avg(
            F.coalesce(F.col("air_cp_kw"), F.col("air_cp_watts") / WATTS_PER_KW)
        ).alias("avg_ac_kw"),
...
insight_06 = (
    trips_enriched.where(
        (F.col("eng_dis").isNotNull()) & (F.col("fuel_rate_est_lhr").isNotNull())
    )
    .groupBy("eng_dis")
    .agg(
        F.count("*").alias("rows_count"),
...
insight_07 = (
    trips_enriched.where((F.col("lat").isNotNull()) & (F.col("_long").isNotNull()))
...
window_spec = Window.partitionBy("vehid", "tripid").orderBy(
    F.col("time_seconds"), F.col("time")
)

trips_for_features = (
    trips_enriched.withColumn(
        "next_time_seconds", F.lead("time_seconds").over(window_spec)
    )
    ...
    .withColumn(
        "dt_s",
        F.when(
            (F.col("dt_s") > 0) & (F.col("dt_s") < MAX_VALID_DT_S), F.col("dt_s")
        ).otherwise(F.lit(0.0)),
    )
    .withColumn("speed_clean", F.coalesce(F.col("speed"), F.lit(0.0)))
    .withColumn(
        "distance_km_segment", F.col("speed_clean") * F.col("dt_s") / F.lit(3600)
    )
    .withColumn(
        "fuel_l_segment",
        F.when(
    ...
    .withColumn(
        "stop_go_s",
        F.when(F.col("speed_clean") < STOP_GO_SPEED_KMH, F.col("dt_s")).otherwise(
            F.lit(0.0)
        ),
    )
    .withColumn(
        "idle_s",
        F.when(F.col("speed_clean") < IDLE_SPEED_KMH, F.col("dt_s")).otherwise(
            F.lit(0.0)
        ),
    )
)
...
trip_features = (
    trips_for_features.groupBy("vehid", "tripid")
    .agg(
        F.min("daynum").alias("daynum"),
        F.count("*").alias("n_sensor_points"),
        ((F.max("time_seconds") - F.min("time_seconds")) / F.lit(60)).alias(
            "duration_min"
        ),
        ...
        F.sum(
            F.when(F.col("fuel_rate_est_lhr").isNotNull(), F.lit(1)).otherwise(F.lit(0))
        ).alias("fuel_valid_points"),
        F.sum("stop_go_s").alias("stop_go_s"),
        F.sum("idle_s").alias("idle_s"),
        F.avg("speed").alias("speed_mean"),
        F.expr(f"percentile_approx(speed, {0.5})").alias("speed_median"),
        F.expr(f"percentile_approx(speed, {0.95})").alias("speed_p95"),
        F.avg("maf").alias("maf_mean"),
        F.expr(f"percentile_approx(maf, {0.95})").alias("maf_p95"),
        F.avg("rpm").alias("rpm_mean"),
        F.expr(f"percentile_approx(rpm, {0.95})").alias("rpm_p95"),
    ...
    .withColumn(
        "stop_go_ratio",
        F.when(
            F.col("observed_seconds") > 0,
            F.col("stop_go_s") / F.col("observed_seconds"),
        ),
    )
    ...
    .withColumn(
        ...
        F.when(
            F.col("distance_km") > 0,
            F.lit(100) * F.col("fuel_used_l") / F.col("distance_km"),
        ),
    )
...
trip_features_sample = trip_features.orderBy(F.desc("distance_km")).limit(
    TRIP_FEATURE_SAMPLE_SIZE
)
